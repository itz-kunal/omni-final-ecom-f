...
import { APPROVE_TRANSACTIONS, GET_TRANSACTIONS } from '@/utils/apiroutes';
...
function DataTable({ transactions = data }) {
    const { toast } = useToast()

    const approveTransaction = async (id, status) => {
        try {
            const res = await axios.post(APPROVE_TRANSACTIONS, { transactionId: id, status }, { withCredentials: true });
            console.log(res)
            toast({
                    ...
                    {transactions.map((transaction, i) => {
                        return (
                            <TableRow key={i}>
                                ...
                                <TableCell>{transaction.upi}</TableCell>
