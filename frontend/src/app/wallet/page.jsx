...
function Page() {
    ...
    const { user, setUser, withdrawls, setWithdrawls } = useContext(UserContext);
    ...
    useEffect(() => {
        async function fetchData() {
            if (user) {
                try {
                    const withdrawlData = await axios.get(WITHDRAWLS, { withCredentials: true });
                    setWithdrawls(withdrawlData.data || []);
            ...
            try {
                const [userData, withdrawlData] = await Promise.all([
                    axios.get(GET_USER, { withCredentials: true }),
                    axios.get(WITHDRAWLS, { withCredentials: true })
                ])
                setUser(userData.data);
                console.log(withdrawlData.data, withdrawls)
                setWithdrawls(withdrawlData.data);

                const totalWithdrawlRes = withdrawlData.data.reduce((acr, cur) => {
